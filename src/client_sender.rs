...
mod tests {
    ...
    async fn client_sender_multiple_clones_share_state() {
        ...
        assert_eq!(
            first_task.await.unwrap().unwrap().result().cloned(),
            Some(json!("first"))
        );
        assert_eq!(
            second_task.await.unwrap().unwrap().result().cloned(),
            Some(json!("second"))
        );
    }
    ...
    async fn client_sender_wraparound_does_not_reuse_pending_id() {
        ...
        assert_eq!(
            max_task.await.unwrap().unwrap().result().cloned(),
            Some(json!("second"))
        );
        assert_eq!(
            wrapped_task.await.unwrap().unwrap().result().cloned(),
            Some(json!("third"))
        );
    }
